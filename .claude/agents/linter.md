...
- Inline `style` is used for static values that should be in a CSS class; when moving the value to CSS, scope it to the context selector (e.g. `.row-date .freq-badge{margin-left:6px}`) rather than mutating a shared base class used in other contexts
  - Specific recurring offender — the "modal header" large-icon variant: `<div class="row-item" style={{ borderBottom: 'none', paddingBottom: 0 }}>` wrapping `<div class="row-icon" style={{ background: color + '22', width: '48px', height: '48px', fontSize: '22px' }}>` with `<div class="row-title" style={{ fontSize: '16px' }}>`. Only `background: color + '22'` is dynamic (derived from `catColor`); every other value (`width`/`height`/`fontSize`/`borderBottom`/`paddingBottom`) is static and must move to a CSS class. This block was copy-pasted from `DetailModal.tsx` into `CatBreakdownModal.tsx`, so it now lives in two files. Flag it for the coder to extract a shared modifier class (e.g. `.row-item.modal-head` + `.modal-head .row-icon` + `.modal-head .row-title` in `components.css`) and apply the class in both modals, keeping only the dynamic `background` inline. Do NOT fix inline yourself — it spans both files and requires new CSS rules (feature work, > 5 lines).
