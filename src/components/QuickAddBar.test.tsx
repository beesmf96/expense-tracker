...
describe('QuickAddBar', () => {
  ...
  it('renders category pills when recentCatIds has entries', () => {
    ...
    userCats.value = [cat]
    txs.value = [makeTx({ category: 'bills_sub' })]
    render(<QuickAddBar />)
...
describe('recentCatIds computed', () => {
  ...
  it('only includes category ids that exist in userCats', () => {
    userCats.value = [makeCat({ id: 'bills_sub' })]
    txs.value = [
      makeTx({ id: 't1', category: 'bills_sub' }),
      makeTx({ id: 't2', category: 'unknown_cat' }),
    ]
  ...
  it('deduplicates: same category used multiple times appears once', () => {
    ...
    txs.value = [
      makeTx({ id: 't1', category: 'bills_sub' }),
      makeTx({ id: 't2', category: 'bills_sub' }),
      makeTx({ id: 't3', category: 'bills_sub' }),
    ]
    expect(recentCatIds.value).toEqual(['bills_sub'])
  })

  it('caps results at 3 unique categories', () => {
    userCats.value = [
    ...
    txs.value = [
      makeTx({ id: 't1', category: 'cat1' }),
      makeTx({ id: 't2', category: 'cat2' }),
      makeTx({ id: 't3', category: 'cat3' }),
      makeTx({ id: 't4', category: 'cat4' }),
    ]
    expect(recentCatIds.value).toHaveLength(3)
    expect(recentCatIds.value).toEqual(['cat1', 'cat2', 'cat3'])
  })
  ...
  it('respects insertion order of txs (first-seen wins)', () => {
    ...
    txs.value = [
      makeTx({ id: 't1', category: 'cat2' }),
      makeTx({ id: 't2', category: 'cat1' }),
    ]
  ...
  it('updates reactively when txs changes', () => {
    ...
    txs.value = [makeTx({ id: 't1', category: 'cat1' })]
  ...
  it('updates reactively when userCats changes', () => {
    userCats.value = []
    txs.value = [makeTx({ id: 't1', category: 'bills_sub' })]
    expect(recentCatIds.value).toEqual([])
